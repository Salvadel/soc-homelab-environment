...
With the Sysmon localfile entry added to ossec.conf and the agent restarted, Sysmon events were confirmed flowing into the Wazuh dashboard. Full details on the Sysmon installation and ossec.conf configuration is documented in [Sysmon Setup](sysmon-setup.md).
