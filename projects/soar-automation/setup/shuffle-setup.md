...
This document covers the installation and configuration of Shuffle on the Ubuntu Server SOAR host. Shuffle is an open source SOAR platform used to automate alert response workflows. It receives alerts from Wazuh via webhook, enriches them with IOC data from VirusTotal and AbuseIPDB, creates cases in TheHive, and sends Slack notifications to the analyst.

## Prerequisites

Before installing Shuffle, ensure the Ubuntu Server SOAR host is fully installed and its static IP is configured. Full details are documented in [Ubuntu Server Setup (SOAR)](ubuntu-soar-setup.md). Internet access through pfSense is required to install Docker and pull the Shuffle Docker images during installation.

## Docker Installation

Docker is required to run Shuffle. Docker was installed on the Ubuntu Server SOAR host using the official Docker convenience script, which automatically detects the OS, adds the repository, and installs Docker in a single command. The official Docker installation guide for Ubuntu can be found at the [Docker Engine Installation Guide](https://docs.docker.com/engine/install/ubuntu/).
```bash
...
Add the local user to the Docker group to run Docker commands without sudo:
```bash
...
Shuffle was installed by cloning the official Shuffle repository and starting the containers using Docker Compose. The official Shuffle repository can be found at [https://github.com/Shuffle/Shuffle](https://github.com/Shuffle/Shuffle).
...
```yaml
- SHUFFLE_WORKER_SERVER_URL=http://192.168.40.40:33333
```
...
On first access, Shuffle prompts you to create an admin account. The following credentials were used:
...
```
http://192.168.40.40:3001/api/v1/hooks/webhook_XXXXXXXX
```

### Step 2 - Configure Wazuh to Forward Alerts

On the core lab's Wazuh Manager, edit the Wazuh Manager configuration file:
```bash
...
<integration>
  <name>shuffle</name>
  <hook_url>http://192.168.40.40:3001/api/v1/hooks/webhook_XXXXXXXX</hook_url>
  <level>7</level>
...
The IP `172.17.0.1` is the Docker bridge gateway address, which is how containers reach services running on the host machine. Using the LAN IP `192.168.40.40` does not work from inside Docker containers.
...
**Resolution:** Waiting 3 to 5 minutes after container startup and retrying the browser resolved the issue.
...
**Resolution:** Set `SHUFFLE_SWARM_CONFIG=off`, `SHUFFLE_WORKER_SERVER_URL=http://192.168.40.40:33333`, and `BASE_URL=http://shuffle-backend:5001` in the orborus service environment section of `docker-compose.yml`, then restarted the containers.
...
- The Shuffle dashboard is accessible at `http://192.168.40.40:3001`
